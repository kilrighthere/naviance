  ...
  <div class="bg-background text-on-background flex h-screen overflow-hidden">
    ...
    >
      ...
      <main class="flex-1 overflow-y-auto overflow-x-hidden px-margin-mobile lg:px-margin-desktop pb-margin-mobile lg:pb-margin-desktop pt-4 w-full">
        ...
        <!-- Data Table Card -->
        <div class="bg-surface-container-lowest rounded-2xl border border-outline-variant/30 shadow-[0_4px_12px_rgba(30,41,59,0.03)] w-full overflow-hidden">
          ...
          <!-- Mobile View (Cards) -->
          <div class="md:hidden flex flex-col divide-y divide-outline-variant/10">
            <div v-if="transaksiStore.isLoading" class="p-8 text-center text-on-surface-variant">
              <span class="material-symbols-outlined animate-spin text-3xl">progress_activity</span>
              <p class="mt-2">Memuat data...</p>
            </div>
            <div v-else-if="!transaksiStore.hasItems || transaksiStore.sortedItems.length === 0" class="p-8 text-center text-on-surface-variant">
              <span class="material-symbols-outlined text-4xl mb-2 text-outline-variant block">receipt_long</span>
              Belum ada transaksi ditemukan.
            </div>
            <div 
              v-else
              v-for="trx in transaksiStore.sortedItems" 
              :key="'mob-' + trx.id_transaksi"
              class="p-4 hover:bg-surface-container-low/50 transition-colors flex flex-col gap-3 group bg-surface-container-lowest"
            >
              <div class="flex justify-between items-start gap-2">
                <div class="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="text-on-surface font-label-md text-label-md truncate">{{ trx.nama_transaksi ?? trx.nama_toko ?? 'Transaksi' }}</div>
                    <div class="text-label-sm font-label-sm text-on-surface-variant/70 capitalize">{{ categoriesMap[trx.id_kategori] || 'Lainnya' }}</div>
                  </div>
                </div>
                <div class="text-right shrink-0">
                  <div class="font-label-md text-label-md" :class="trx.jenis_transaksi === 'pemasukan' ? 'text-on-tertiary-container' : 'text-on-surface'">
                    {{ trx.jenis_transaksi === 'pemasukan' ? '+' : '-' }} Rp {{ trx.nominal.toLocaleString('id-ID') }}
                  </div>
                  <div class="text-label-sm font-label-sm text-on-surface-variant mt-0.5">{{ formatDate(trx.tanggal_transaksi) }}</div>
                </div>
              </div>
              <div class="flex justify-between items-center pt-2 border-t border-outline-variant/10 mt-1">
                <span class="text-label-sm font-label-sm text-on-surface-variant/70 truncate mr-2">{{ trx.deskripsi ?? (trx.nama_toko ? 'Toko' : 'Detail') }}</span>
                <button class="text-on-surface-variant hover:text-primary p-1.5 rounded-full hover:bg-surface-container-high transition-colors shrink-0" @click="openModal('edit', trx)">
